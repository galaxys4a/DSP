...
def get_data_from_analytic_signal(z):
    ...
    a = get_envelope(s, s_ort)
    phi = get_phase(s, s_ort)
    w, w_unwrap = get_frequency(s, s_ort)
    
    return [a, phi, w, w_unwrap]


def get_envelope(s, s_ort):
    a = sqrt(s**2 + s_ort**2)
    return a


def get_phase(s, s_ort):
    phi = []

    for i in xrange(len(s)):
        if s[i] == 0:
            phi.append((pi / 2) * (s_ort[i] / abs(s_ort[i])))
        else:
            phi.append(arctan(s_ort[i] / s[i]))

    return phi


def get_frequency(s, s_ort):
...
z = hilbert(am_signal)
[a, phi, w, w_unwrap] = get_data_from_analytic_signal(z)

draw(xrange(len(a)), a, 'Envelope', 'Envelope')
draw(xrange(len(phi)), phi, 'Phase', 'Phase')
draw(xrange(len(w)), w, 'Frequency', 'Frequency')

draw(xrange(len(w_unwrap)), w_unwrap, 'Frequency(unwrap)', 'Frequency(unwrap)')

# todo: handle DivideByZeroException
phi_unwrap = arctan(unwrap(imag(z) / real(z)))
draw(xrange(len(phi_unwrap)), phi_unwrap, 'Phase(unwrap)', 'Phase(unwrap)')
#
